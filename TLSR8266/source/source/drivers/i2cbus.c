...
void I2C_start_stop(unsigned char flg) {
	u8 r = irq_disable();
   	reg_i2c_ctrl = flg;
    while(reg_i2c_status & FLD_I2C_CMD_BUSY	);
	irq_restore(r);
...
int I2C_dev_addr(unsigned char i2c_addr) {
	int ret;
	u8 r = irq_disable();
	reg_i2c_id = i2c_addr;
	reg_i2c_ctrl = FLD_I2C_CMD_START | FLD_I2C_CMD_ADR;
    while(reg_i2c_status & FLD_I2C_CMD_BUSY	);
	ret = (reg_i2c_status & FLD_I2C_NAK) == 0;
	irq_restore(r);
	return ret;
...
int I2C_wr(unsigned char reg_data, unsigned char flg) {
	int ret;
	u8 r = irq_disable();
	reg_i2c_do = reg_data;
   	reg_i2c_ctrl = FLD_I2C_CMD_DO | flg;
    while(reg_i2c_status & FLD_I2C_CMD_BUSY	);
	ret = (reg_i2c_status & FLD_I2C_NAK) == 0;
	irq_restore(r);
	return ret;
...
unsigned char I2C_rd(unsigned char flg) {
	unsigned char ret;
	u8 r = irq_disable();
	reg_i2c_id |=  FLD_I2C_WRITE_READ_BIT;
	reg_i2c_ctrl = FLD_I2C_CMD_DI | FLD_I2C_CMD_READ_ID | flg;
	while(reg_i2c_status & FLD_I2C_CMD_BUSY);
	ret = reg_i2c_di;
	irq_restore(r);
	return ret;
...
int I2C_Read_A8(unsigned char i2c_addr, unsigned char reg_addr, void *data, int rdlen, unsigned char nak)
{
	...
	if(i) {
		...
		reg_rst0 = tmp;
		reg_i2c_id = i2c_addr;  //SlaveID & 0xfe,.i.e write data. Read:High  Write:Low
		//write addr
...
//		reg_i2c_id &= (~FLD_I2C_WRITE_READ_BIT); // SlaveID & 0xfe,.i.e write data. R:High  W:Low
	...
	reg_i2c_di = *p++;
	if(!i)
		reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP | FLD_I2C_CMD_START | FLD_I2C_CMD_ID;
	else if(!--i)
		reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP | FLD_I2C_CMD_START | FLD_I2C_CMD_ID | FLD_I2C_CMD_ADR;
	else if(!--i)
		reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP | FLD_I2C_CMD_START | FLD_I2C_CMD_ID | FLD_I2C_CMD_ADR | FLD_I2C_CMD_DO;
	else if(!--i)
		reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP | FLD_I2C_CMD_START | FLD_I2C_CMD_ID | FLD_I2C_CMD_ADR | FLD_I2C_CMD_DO | FLD_I2C_CMD_DI;
	else // i > 3
		reg_i2c_ctrl = FLD_I2C_CMD_START | FLD_I2C_CMD_ID | FLD_I2C_CMD_ADR | FLD_I2C_CMD_DO | FLD_I2C_CMD_DI;
	while(reg_i2c_status & FLD_I2C_CMD_BUSY);
	ret = (reg_i2c_status & FLD_I2C_NAK) == 0;
	while(ret && i) {
		...
		reg_i2c_di = *p++;
		if(!--i) {
			reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP;
			break;
		} else if(!--i) {
			reg_i2c_ctrl = FLD_I2C_CMD_ADR | FLD_I2C_CMD_STOP;
			break;
