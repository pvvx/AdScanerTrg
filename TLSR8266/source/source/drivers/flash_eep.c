 ...
 *  Port RTL87xx: 15/10/2016
 *  Port TLSR82xx: 10/11/2019
 *      Author: pvvx
 */
...
#define DBG_FEEP_ERR(...)     do {\
	...
	printf(FEEP_ERR_PREFIX __VA_ARGS__); \
}while(0)
#else
#define DBG_FEEP_ERR(...)
#endif
...
#define DBG_FEEP_WARN(...)     do {\
	...
	printf(FEEP_WARN_PREFIX __VA_ARGS__); \
}while(0)
#else
...
#define DBG_FEEP_INFO(...)     do {\
	...
	printf(FEEP_INFO_PREFIX __VA_ARGS__); \
}while(0)
#else
	...
	if(faddr >= FMEM_ERROR_MAX)  {
		...
		if(xfaddr > FMEM_ERROR_MAX && size == fobj.n.size) {
			if(size == 0
				...
				for(i=0; i < size; i+=8) {
					DBG_FEEP_INFO("buf[%d]\t%02X %02X %02X %02X  %02X %02X %02X %02X\n",
								i, r[i], r[i+1], r[i+2], r[i+3], r[i+4], r[i+5], r[i+6], r[i+7]);
					DBG_FEEP_INFO("obj[%d]\t%02X %02X %02X %02X  %02X %02X %02X %02X\n",
